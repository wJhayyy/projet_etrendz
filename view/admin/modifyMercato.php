...
// Assurez-vous que $all_mercato[0]['date'] contient la date correcte depuis la base de données
$dateFromDB = $all_mercato[0]['date_mercato'];
    ...
    <form method="POST" action="index.php?admin=editMercato" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
            <div>
                ...
                <input id="titre" name="titre" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" placeholder="Titre de l'actualité" value="<?php echo htmlspecialchars($all_mercato[0]['titre']); ?>">
            </div>
            <div>
                <label class="block text-sm font-medium text-white">
                Image Entete : 
              </label>
              <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md backgroundDiv" id="image_entete_div" style="background: url(assets/upload/<?php echo $all_mercato[0]['image_entete']; ?>); background-size: 100%; background-position: center center;">
            ...
            <div>
                ...
                <select name="category" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                    <?php foreach ($all_filter as $filter) : ?>
                        <?php
                        // Vérifier si l'ID de la catégorie correspond à l'ID du mercato à modifier
                        $selected = ($filter['id_category'] == $all_mercato[0]['id_category']) ? 'selected' : '';
                        ?>
                        <option value="<?php echo htmlspecialchars($filter['id_category']) ?>" <?php echo $selected ?>>
                            <?php echo htmlspecialchars(str_replace(' - Mercato', '', $filter['name_category'])) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
