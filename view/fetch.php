...
?>
    ...
    <?php foreach ($all_filter as $filter) : ?>
        <div class="w-5/6 lg:flex">
            <div class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden" style="background-image: url('assets/image/<?php echo htmlspecialchars($filter['image_entete']); ?>')" title="<?php echo htmlspecialchars($filter['titre_actualite']); ?>">
            </div>
            <div class="max-w-full w-96 border-r border-b border-l border-grey-light lg:border-l-0 lg:border-t lg:border-grey-light bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                <div class="mb-8">
                    <div class="text-black font-bold text-xl mb-2"><a href="<?php echo htmlspecialchars('index.php?action=actualité?id=' . $filter['id_actualite']); ?>"><?php echo limitText(htmlspecialchars($filter['titre_actualite']),60); ?></a></div>
    ...
    <?php foreach ($all_mercato as $mercato_filter) : ?>
        <div class="w-5/6 lg:flex">
                    <div class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden" style="background-image: url('assets/image/<?php echo htmlspecialchars($mercato_filter['image_entete']); ?>')" title="<?php echo htmlspecialchars($mercato_filter['titre']); ?>">
                    </div>
                    <div class="max-w-full w-96 border-r border-b border-l border-grey-light lg:border-l-0 lg:border-t lg:border-grey-light bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                        <div class="mb-8">
                        <div class="text-black font-bold text-xl mb-2"><a href="<?php echo htmlspecialchars('index.php?action=mercato?id=' . $mercato_filter['id_mercato']); ?>"><?php echo limitText(htmlspecialchars($mercato_filter['titre']),60); ?></a></div>
